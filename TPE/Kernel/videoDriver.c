...
/*
 * Matriz extraida de la guia de arduino
 */
char fonts[130][8] = {
...
static ModeInfoBlock *infoBlock = (ModeInfoBlock*)0x5C00;
...
/*
 * Hacer for i for j de putpixel para rectangulo es poco eficiente,
 * porque tiene que recalcular screen muchas veces
 */
void fillRect(unsigned char x, unsigned char y, uint16_t width, uint16_t height, unsigned char r, unsigned char g, unsigned char b) {
...
/*
 * Esta funcion difiere de drawchar porque no hace el calculo de pos*,
 * el drawString lo calcula y ya cambia automaticamente la posicion
...
/*
 * Las fonts son arrays de 8 bytes, cada bit es una posicion que vale 0 o 1, son 64 pixels descriptos
 * Vamos a pintar los pixels que tienen un 1 con el color que viene por parametro,
 * y de negro los que no vienen por parametro
 */
void drawChar(int  x, int  y, unsigned char myChar, unsigned char r, unsigned char g, unsigned char b) {
...
void backspace() {
  if (cursorX > cursorXStart){
    cursorX -= 8;                           //Cada letra ocupa 8 "lugares"
    drawChar(cursorX, cursorY, 129,0,0,0);  //129 es el ultimo elemento de la matriz font = {FF,FF,FF,FF,FF,FF,FF,FF} => todos 1 => tapa lo que habia con negro
  }
}
